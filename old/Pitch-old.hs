...
instance HasPitch NoteP where
  pitch (NoteP p _) = pitch p

(=~) :: (HasPitch a) => a -> a -> Bool
a =~ b = pitchRat a == pitchRat b

-- instance HasPitch NoteP where
--   pitch (NoteP noteName accidental octave) = Pitch noteName accidental octave
...
-- ghci> c 0 t4
-- NoteT (Pitch C Natural (Octave 0)) Quarter

note1 = NoteP cs' Quarter
note2 = NoteP df' Quarter
...
-- pNote :: Parser MusicElement
-- pNote = do
--   pitch <- parsePitch
--   dur <- parseDur
--   return (Note pitch dur)

-- note :: String -> Either ParseError MusicElement
-- note str = parse pNote "" str

-- test :: IO ()
-- test = do
--     let result = parse note "" "c4"
--     case result of
--         Left err -> putStrLn ("Parsing failed: " ++ show err)
--         Right musicElement -> putStrLn ("Parsed music element: " ++ show musicElement)
